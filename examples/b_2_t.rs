...
fn main() {
    ...
    let mut file = File::create(format!("out/systems/{}.txt", name)).unwrap();
    let energies_dict = get_energies_dict(energies);

    writeln!(file, "{}", energies_dict).unwrap();
    writeln!(file, "{},{},{}", WIDTH, HEIGHT, STEPS).unwrap();
    writeln!(file, "{:?}", concentration.get_cs()).unwrap();
